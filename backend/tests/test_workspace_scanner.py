...
def test_build_workspace_path_context_for_missing_directory(
    monkeypatch,
    tmp_path: Path,
) -> None:
    ...
    context = build_workspace_path_context(intent)
    assert "GitHub/NewProject" in context
    assert "does not exist yet" in context


def test_build_workspace_path_context_lists_existing_files(
    monkeypatch,
    tmp_path: Path,
) -> None:
